  ...
  <div class="container">
    ...
    <table class="table table-bordered text-center">
      <tr>
        <td
          :key="item.id"
          v-for="item in getKoltukList('Çiftli', 'Cam Kenarı')"
        >
          <button
            :class="{
              koltuk: item.durum === 0,
              'koltuk koltuk-dolu': item.durum === 1,
              'koltuk koltuk-rez': item.durum === 2,
            }"
      ...
      <tr>
        <td :key="item.id" v-for="item in getKoltukList('Çiftli', 'Koridor')">
          <button
            :class="{
              koltuk: item.durum === 0,
              'koltuk koltuk-dolu': item.durum === 1,
      ...
      <tr>
        <td :key="item.id" v-for="item in getKoltukList('Tekli', 'Cam Kenarı')">
          <button
            :class="{
              koltuk: item.durum === 0,
              'koltuk koltuk-dolu': item.durum === 1,
    ...
    <button
      ...
      @click="yolcu_bilgilerini_gir"
      v-if="secilen_koltuklar.length !== 0"
    >
      Yolcu Bilgilerini Giriniz
    </button>
  </div>
</template>
...
export default {
  ...
  methods: {
    fetchKoltuklar() {
      ...
      else this.$router.push({ name: "seferara" });

      this.secilen_sefer = this.seferler.find((x) => x.id === sefer_id);
      this.bulunan_koltuklar = this.koltuklar.filter(
        (x) => x.sefer_id === sefer_id
      );
    },
    getKoltukList(tip, konum) {
      return this.bulunan_koltuklar.filter(
        (k) => k.tip === tip && k.konum === konum
      );
    },
    koltuk_sec(koltuk_id) {
      ...
      if (koltuk.durum === 0)
        this.secilen_koltuklar = this.secilen_koltuklar.filter(
          (k) => k.id !== koltuk_id
        );
      else this.secilen_koltuklar.push(koltuk);
    },
