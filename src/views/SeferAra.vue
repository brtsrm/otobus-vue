  ...
  <div class="container">
      ...
      <div class="form-row align-items-center">
        <div class="col-sm-2">
          ...
          <select class="form-control" v-model="kalkis_noktasi">
            ...
            <option
              v-for="item in hareket_noktalari"
              :value="item.id"
              :key="item.id"
              v-text="item.aciklama"
            ></option>
          </select>
        </div>
        <div class="col-sm-2">
    ...
    <div v-if="bulunan_seferler.length">
      ...
      <table class="table table-hover">
        <tr :key="index" v-for="(item, index) in bulunan_seferler">
          <td>
            Kalkış Saati
...
export default {
  ...
  methods: {
    sefer_ara() {
      this.bulunan_seferler = this.seferler.filter(
        (x) =>
      ...
      if (this.bulunan_seferler.length === 0) {
        this.mesaj = "Bir sefer bulunmadı";
      }
